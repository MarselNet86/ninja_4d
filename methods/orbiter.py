...
async def orbiter_eth_to_scroll(wallet):
    web3 = Web3(Web3.HTTPProvider(rpc['eth_goerli']))

    amount_transfer = round(random.uniform(orbiter_down, orbiter_up), 5)
    ...
    print(f'--> Orbiter Bridge | Goerli to Scroll | Баланс: {amount_balance} ETH | Сумма перевода: {amount_transfer} ETH')
...
async def orbiter_scroll_to_eth(wallet):
    ...
    print(f'--> Orbiter Bridge | Scroll to Goerli | Баланс: {amount_balance} ETH | Сумма перевода: {amount_transfer} ETH')
